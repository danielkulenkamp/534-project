...
#                 not(has_edge(G100, nodes[i], nodes[3])) or \
#                 not(has_edge(G100, nodes[i], nodes[4])):
#                 complete = False
...
def find_five_complete(node_dir, plot=False):
    ...
    p = Pool(12)
    p.map(process_perm, itertools.permutations(G100, 5))
...
@cmd 
def find_hidden_terminal(node_dir, plot=False):
    if isinstance(plot, basestring):
        plot = bool(strtobool(plot))
    ...
    for nodes in itertools.permutations(G100, 4):
        hidden = True

        if not(has_edge(G100, nodes[0], nodes[1])) or \
                not(has_edge(G100, nodes[0], nodes[2])) or \
                has_edge(Gall, nodes[0], nodes[3]):
            hidden = False
        
        if not(has_edge(G100, nodes[1], nodes[0])) or \
                not(has_edge(G100, nodes[1], nodes[2])) or \
                not(has_edge(G100, nodes[1], nodes[3])):
            hidden = False
        
        if not(has_edge(G100, nodes[2], nodes[0])) or \
                not(has_edge(G100, nodes[2], nodes[1])) or \
                not(has_edge(G100, nodes[2], nodes[3])):
            hidden = False
        
        if not(has_edge(G100, nodes[3], nodes[1])) or \
                not(has_edge(G100, nodes[3], nodes[2])) or \
                has_edge(Gall, nodes[3], nodes[0]):
            hidden = False
...
@cmd
def find_butterfly(node_dir, plot=False):
    if isinstance(plot,basestring):
        plot = bool(strtobool(plot))
    ...
    for nodes in itertools.permutations(G100, 6):
        butterfly = True

        # for node 0
        if not(has_edge(G100, nodes[0], nodes[1])) or \
                not(has_edge(G100, nodes[0], nodes[3])) or \
                not(has_edge(G100, nodes[0], nodes[4])) or \
                has_edge(Gall, nodes[0], nodes[2]) or \
                has_edge(Gall, nodes[0], nodes[5]):
            butterfly = False

        if not(has_edge(G100, nodes[1], nodes[0])) or \
                not(has_edge(G100, nodes[1], nodes[2])) or \
                not(has_edge(G100, nodes[1], nodes[3])) or \
                not(has_edge(G100, nodes[1], nodes[4])) or \
                not(has_edge(G100, nodes[1], nodes[5])):
            butterfly = False

        if not(has_edge(G100, nodes[2], nodes[1])) or \
                not(has_edge(G100, nodes[2], nodes[4])) or \
                not(has_edge(G100, nodes[2], nodes[5])) or \
                has_edge(Gall, nodes[2], nodes[0]) or \
                has_edge(Gall, nodes[2], nodes[3]):
            butterfly = False
        
        if not(has_edge(G100, nodes[3], nodes[0])) or \
                not(has_edge(G100, nodes[3], nodes[1])) or \
                not(has_edge(G100, nodes[3], nodes[4])) or \
                has_edge(Gall, nodes[3], nodes[2]) or \
                has_edge(Gall, nodes[3], nodes[5]):
            butterfly = False
                    
        if not(has_edge(G100, nodes[4], nodes[0])) or \
                not(has_edge(G100, nodes[4], nodes[1])) or \
                not(has_edge(G100, nodes[4], nodes[2])) or \
                not(has_edge(G100, nodes[4], nodes[3])) or \
                not(has_edge(G100, nodes[4], nodes[5])):
            butterfly = False
        
        if not(has_edge(G100, nodes[5], nodes[1])) or \
                not(has_edge(G100, nodes[5], nodes[2])) or \
                not(has_edge(G100, nodes[5], nodes[4])) or \
                has_edge(Gall, nodes[5], nodes[0]) or \
                has_edge(Gall, nodes[5], nodes[3]):
            butterfly = False
